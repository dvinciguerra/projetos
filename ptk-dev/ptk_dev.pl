...
my $mw=MainWindow->new(
    -title=>'[No Saved*] - Perl TK Development'
);
...
my $cmd_phelp = $wm_ajuda -> command (
    -label=>'   Topicos de Ajuda do Programa      F1'
);
$wm_ajuda->separator();
my $cmd_plreferences = $wm_ajuda -> command (
    -label=>'   Referencias (Perl)'
);
my $cmd_ptkreferences = $wm_ajuda -> command (
    -label=>'   Referencias (Perl/Tk)'
);
$wm_ajuda->separator();
...
my $btn_new = $Toolbar -> Button ('-image'=>'img_new', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_new,-balloonmsg=>"Cria um novo projeto");
$mw->Photo('img_open', -file=>$open_img, '-format'=>'PNG');
my $btn_open = $Toolbar -> Button ('-image'=>'img_open', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_open,-balloonmsg=>"Abre um projeto");
$mw->Photo('img_save', -file=>$save_img, '-format'=>'PNG');
my $btn_save = $Toolbar -> Button ('-image'=>'img_save', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_save,-balloonmsg=>"Salva o projeto atual");
my $lbl_space1 = $Toolbar -> Label ( -foreground=>'gray50', -justify=>'left', -relief=>'flat', -text=>'|' ) -> pack(-side=>'left', -padx=>2);
$mw->Photo('img_undo', -file=>$undo_img, '-format'=>'PNG');
my $btn_undo = $Toolbar -> Button ('-image'=>'img_undo', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>'OO', -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_undo,-balloonmsg=>"Desfaz a ultima ação");
$mw->Photo('img_redo', -file=>$redo_img, '-format'=>'PNG');
my $btn_redo = $Toolbar -> Button ('-image'=>'img_redo', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_redo,-balloonmsg=>"Refaz a ultima ação");
my $lbl_space2 = $Toolbar -> Label ( -foreground=>'gray50', -justify=>'left', -relief=>'flat', -text=>'|' ) -> pack(-side=>'left', -padx=>2);
$mw->Photo('img_cut', -file=>$cut_img, '-format'=>'PNG');
my $btn_cut = $Toolbar -> Button ('-image'=>'img_cut', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_cut,-balloonmsg=>"Recorta um widget");
$mw->Photo('img_copy', -file=>$copy_img, '-format'=>'PNG');
my $btn_copy = $Toolbar -> Button ('-image'=>'img_copy', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_copy,-balloonmsg=>"Copia um widget");
$mw->Photo('img_paste', -file=>$paste_img, '-format'=>'PNG');
my $btn_paste = $Toolbar -> Button ('-image'=>'img_paste', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_paste,-balloonmsg=>"Cola um widget");
my $lbl_space3 = $Toolbar -> Label ( -foreground=>'gray50', -justify=>'left', -relief=>'flat', -text=>'|' ) -> pack(-side=>'left', -padx=>2);
$mw->Photo('img_run', -file=>$run_img, '-format'=>'PNG');
my $btn_run = $Toolbar -> Button ('-image'=>'img_run', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_run,-balloonmsg=>"Executa a projeto atual");
$mw->Photo('img_source', -file=>$source_img, '-format'=>'PNG');
my $btn_source = $Toolbar -> Button ('-image'=>'img_source', -overrelief=>'flat', -state=>'normal', -relief=>'flat', -text=>44, -compound=>'none' ) -> pack(-side=>'left', -padx=>2);
$tooltip->attach($btn_source,-balloonmsg=>"Exibe o código fonte do projeto atual");
...
my $w_Label_020 = $Statusbar -> Label (
    -justify=>'left',
    -relief=>'flat',
    -text=>'Perl Tk Development IDE version 0.1a - Daniel Vinciguerra <daniel-vinciguerra@hotmail.com>'
) -> pack(
    -side=>'left'
);
MainLoop;
...
###############################################
sub label {
    my $widget_str = "my \$mw_label0$c_label = \$Main -> Label (-text => 'Label $c_label') -> place( -x=>0, -y=>0 );";
    $c_label++;
    # print $widget_str;
    eval($widget_str);
}
...
sub button {
    my $widget_str = "my \$mw_button0$c_button = \$Main -> Button (-text => 'Button $c_button' ) -> place( -x=>0, -y=>0 );";
    $c_button++;
    # print $widget_str;
    eval($widget_str);
}
